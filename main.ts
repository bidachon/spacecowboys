...
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.tree, function (sprite, otherSprite) {
    projectile.setVelocity(projectile.vx * (Math.randomRange(-50, 50) * -1), projectile.vy * (Math.randomRange(-50, 50) * -1))
})
function maketrees () {
    let alltrees: Sprite[] = []
...
`, SpriteKind.tree)
    ...
    for (let value of alltrees) {
        value.setPosition(Math.randomRange(10, scene.screenWidth()), Math.randomRange(10, scene.screenHeight()))
    }
}
